...
class WP_Members_Forms {
	...
	function login_form( $page, $arr ) {
		...
		$defaults = array(
			...
			'main_div_after'  => '</div>',
			'txt_before'      => '<div class="error-msg"> This content is restricted to site members.</br>If you are an existing user, please log in.  New users may register below.</div>',
			'txt_after'       => '',
			'row_before'      => '',
			'row_after'       => '',
		...
		if ( $arr['action'] == 'login' ) {
			...
			$buttons =  $args['remember_check'] . $args['t'] . '<input type="submit" name="cancel" value="Cancel" onclick="window.history.go(-1); return false;" class="' . $args['button_class'] . '" />' .'<input type="submit" name="Submit" value="' . $arr['button_text'] . '" class="' . $args['button_class'] . '" />'. $args['n'];
		} else {
	...
	function register_form( $tag = 'new', $heading = '', $redirect_to = null ) {
		...
		$buttons.= '<input name="cancel" type="submit" onclick="window.history.go(-1); return false;" value="Cancel" class="' . $args['button_class'] . '" />' . '<input name="submit" type="submit" value="' . $button_text . '" class="' . $args['button_class'] . '" />' . $args['n'];
